...
function connect() {
    let opacity = 1;
    for (let aa = 0; aa < particleArray.length - 1; aa++){
        for (let bb = aa + 1; bb < particleArray.length; bb++){
            let dx = particleArray[aa].x - particleArray[bb].x;
            let dy = particleArray[aa].y - particleArray[bb].y;
            let distance = Math.sqrt(dx*dx + dy*dy);
            let connectDistance = canvas36.width * canvas36.height / connectDensity;
            if (distance < connectDistance) {
                opacity = 1 - distance / connectDistance / 2;
                ctx36.strokeStyle = lineColor + ", " + opacity + ")";
                ctx36.lineWidth = lineWidth;
                ctx36.beginPath();
                ctx36.moveTo(particleArray[aa].x, particleArray[aa].y);
                ctx36.lineTo(particleArray[bb].x, particleArray[bb].y);
                ctx36.stroke();
            }
        }
    }
